...
if ($result = mysqli_query($con, $sql)) {
	if (mysqli_num_rows($result) > 0) {
		...
		if (time() - strtotime($row['loggedIn']) < 10) {
			...
			$sql2 = "SELECT userID, username, paidPointsBalance, freeConvertablePointsBalance, freeUnconvertablePointsBalance, email, emailConfirmed, notificationsArray, timeNotificationsViewed, numQuizzesTakenRemaining, quizMaster, mobile FROM Users WHERE username = '$username'";
